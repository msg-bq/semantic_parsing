...
class PointAttention(nn.Module):
    ...
    def forward(self,hidden1_states,hidden2_states,lm_logits,tags = [[0,0,1,2,-1,3,4,5,-1]]):
        ...
        # 尝试给merge扩维
        TopK_Shape = hidden1_states.shape[0]
        Shape = merge.shape[0]
        k = TopK_Shape/Shape
        if(k>1):
            merge1 = merge.repeat_interleave(int(k),dim=0)
            finalBias1 = finalBias.repeat_interleave(int(k),dim=0)
        else:
            merge1 = merge
            finalBias1 = finalBias
        # merge = merge.repeat_interleave(,dim=0)
        attn_weights = torch.matmul(merge1,hidden1_states) # hidden1_states1 8 512【300】 728

        attn_weights1 = torch.matmul(hidden2_states, attn_weights.transpose(1, 2))+finalBias1   # hidden2_states 8 512 768  attn_weights 8 512 768  attn_weights1 8 512 512
...
# 为列表中的每个元素分配序号
# 中文没考虑数字
# def label_words(original_sentence,word_list):
#     indices = []
#     num = 0
#     for index, word in enumerate(word_list):
#         if word in special_tokens:  # 特殊字符检查
#             indices.append(-1)
#             num += 1
#         else:
#             indices.append(index-num)
    
#     return indices

# 英文拼接
# def label_words(original_sentence,tokenized_words):
#     labels = []
#     label = 0
#     # word_index = 0
#     current_word = ""
#     lastword = ""
#     # 判断是不是最后一个句号
#     period = original_sentence.count(".")
#     period_num = 1
#     # print(tokenized_words)
#     for word in tokenized_words:
#         # 如果当前单词是非字母（如空格或特殊符号），单独分配一个标签
#         pmFlag = False
#         if not word.isalpha() and not word.isdigit() and word.find("'") == -1 and not is_valid_time_format(word):
#             # .不能丢。如果是句子中的最后一个，则... 如果不是句子中的最后一个。则...
#             if word == "." and period_num < period:
#                 period_num += 1
#             elif word == "." and period_num == period:
...
#         if lastword.isdigit() and original_sentence.find(lastword + word) != -1 and (word.lower() == "pm" or word.lower() == "am"):
#             pmFlag = True
#         # 检查当前累加的单词片段是否与原句中的单词相匹配
#         # print(original_sentence.split())

#         if current_word in original_sentence.split() or pmFlag == True:
#             if pmFlag == True:
#                 label+=1

#             if current_word in original_sentence.split():
#                 current_word = ""
...
#         # 如果当前累加的片段已经形成了完整的单词，则增加标签值
#         if current_word == "":
#             label += 1
#         lastword = word
#     # return changeModel(labels)
#     return labels
...
class MT5ForConditionalGeneration(MT5PreTrainedModel):
    ...
    def __init__(self, config: MT5Config):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/data/lbq/models/mt5-base-trained-final-500+500-2-7_again") # ("/home/lzx/T5-base/model3/mt5-base-trained-final-500+500-2-7_again")#
        # Model parallel
        self.model_parallel = False
        self.device_map = None
    ...
    def _resize_token_embeddings(self, new_num_tokens):
        ...
        if self.get_output_embeddings() is not None and not self.config.tie_word_embeddings:
            ...
            #512
            new_lm_head = self._get_resized_lm_head(old_lm_head, new_num_tokens-128)
    ...
    def forward(
        ...
        lm_logits1 = self.lm_head(sequence_output)
        ...
        # print(cross_attentions.shape)
        # lm_logits = torch.cat([lm_logits, cross_attentions], dim=2)
        # print(lm_logits.shape)
        # eps = 1e-7
        # lm_logits = torch.log(lm_logits + eps)
        lm_logits = self.pointNet(hidden_states,sequence_output,lm_logits1,self.tags)
        # print(labels)
        # print(lm_logits)
