...
from RobertaBiLinearAttentionModel import RobertaBiLinearAttentionModel
...
if __name__ == "__main__":
    ...
    import json
    with open('/home/lzx2000/test/low_resources_semantic_parsing/add_tokens.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
    other_tokens = list(data.keys())
    tokenizer.add_tokens(other_tokens)
    model._resize_token_embeddings(len(tokenizer))

    dataset = load_dataset("/home/lzx2000/test/low_resources_semantic_parsing/data/top_train")
    ...
    # print(ids)
    # # 使用模型生成文本
    # outputs = model.generate(ids, max_length=64)
    #
    # # 解码生成的token为文本
